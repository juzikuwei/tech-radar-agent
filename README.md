...
- **多种接入方式**：React 聊天界面（回答通过引用校验后经 SSE 分块推送）、FastAPI HTTP API 和带鉴权的只读 MCP Server。
...
运行依赖与开发依赖（pytest 等）分列在 `requirements.txt` 和 `requirements-dev.txt`；只部署运行时可跳过后者。首次加载嵌入模型和 Cross-encoder 时可能需要下载模型文件。
...
调用 arXiv 官方 API 拉取一个小批次，并将命令输出的快照路径用于后续导入：
...
论文数据全部通过 arXiv 官方 API 拉取，拉取是手动批处理；在线问答不会实时调用 arXiv。`data/` 中的快照、SQLite 数据库和 ChromaDB 索引默认不提交到 Git。
...
脚本会启动 FastAPI 和 Vite，并打开 `http://127.0.0.1:5173`。服务在后台运行：进程 ID 记录在 `.run/`，输出写入 `logs/`，用 `.\stop_services.ps1` 停止。也可以分别启动：
...
5. **证据边界由代码维护。** 最终研究回答必须至少包含一个本轮证据中的方括号引用；回答任意位置出现未知 arXiv ID 都会整答拒绝。论文卡片、持久化 ID、活动证据和行内超链接只使用校验通过的实际引用；网页片段、模型记忆、旧助手回答和压缩摘要都不是论文证据。
6. **Agent 失败不等于请求失败。** 模型或 harness 出现不可恢复错误时，外层会降级到可靠的固定 Agentic RAG 管线，并保留降级 Trace。
7. **可观测性停在有意义的边界。** ReAct Trace 保留模型调用、工具生命周期、错误、usage 和最终输出，但 React 默认将开始/完成事件合并为产品级步骤；完整原始事件仍可在技术详情中展开。E5、BM25、RRF 等内部细节留在检索实现中，避免把一个逻辑工具拆成模型无法控制的伪工具。
...
| `POST` | `/conversations/{id}/chat/stream` | 通过 SSE 发送状态、工具事件、校验后文本分块、usage 和最终结果 |
...
质量评测与 pytest 分开运行。`smoke` 和 `retrieval` 只加载本地知识库与本地模型；`agent`、`answer` 和 `memory` 会调用 `.env` 中配置的回答模型。生成报告保存在已忽略的 `eval/results/`，默认按套件名加时间戳命名，也可用 `--output` 指定路径：
...
frontend/       React 聊天界面、可信引用链接、引用卡片与精简 Trace
ingestion/      arXiv API 拉取、规范化、快照与 SQLite 导入
...
- 数据更新由人工触发，没有定时拉取或在线增量更新。
- ReAct 最多调用 5 次工具；两种模式都记录并返回本次请求的累计 token usage（降级时合并 Agent 与管线的消耗），尚未加入请求级 token 上限。
...
- 确定性校验能够保证引用 ID 来自本轮证据，但不能证明每一句结论都被对应摘要完整蕴含；更细粒度的 claim-level groundedness 仍需要后续评估或受控 Reflection。
- 尚未引入跨会话长期记忆、LangGraph、Human-in-the-Loop 或多 Agent 编排。

下一阶段计划是用真实长会话校准压缩阈值，并验证早期用户约束在多次批量压缩后仍能稳定保留。重要架构取舍见 [docs/decision-log.md](docs/decision-log.md)。
