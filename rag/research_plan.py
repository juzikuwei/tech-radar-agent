...
ResearchActionType = Literal["search_papers", "web_search", "finish", "refuse"]
...
4. 每轮只能调用一个工具，或者选择 finish/refuse 终止。search_papers 的目标子问题在本轮必须保持 pending 或 unresolved，不得同时标记为 covered。
...
只输出以下 JSON 对象：
{
  ...
  "next_action": {
    "type": "search_papers | web_search | finish | refuse",
...
def _parse_action(value: dict[str, object]) -> ResearchAction:
    ...
    if action_type not in {"search_papers", "web_search", "finish", "refuse"}:
